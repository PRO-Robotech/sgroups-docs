...
**UniRule** — универсальное правило SGroups, единый метод описания сетевых политик для всех
комбинаций эндпоинтов. Тип правила определяется парой `endpoints.local.type` и `endpoints.remote.type`.
Для локальной стороны допустимы `ADDRESS_GROUP` и `SERVICE`, для удалённой — `ADDRESS_GROUP`,
`SERVICE`, `FQDN` и `CIDR` (итого 8 комбинаций). Именно UniRule транслируется в конечные правила nftables.
...
Обе стороны — адресные группы. Самый распространённый сценарий.
...
Локальная сторона — адресная группа, удалённая — сервис.
...
Локальная сторона — сервис, удалённая — адресная группа.
...
Локальная сторона — адресная группа, удалённая — доменное имя.
...
Локальная сторона — сервис, удалённая — доменное имя.
...
Локальная сторона — адресная группа, удалённая — IP-подсеть.
Поле `value` обязательно, `name` и `namespace` у remote не указываются.
...
Локальная сторона — сервис, удалённая — IP-подсеть.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>DNS-разрешённые IP добавляются в динамический set: <code>@fqdn-&#123;domain&#125;</code></td>
    </tr>
    <tr>
    ...
    <tr>
      <td><strong>Action: Deny</strong></td>
...
Удалённая сторона — сервис. Сервис резолвится через ServiceBinding в адресные группы,
...
Удалённая сторона — FQDN. Агент периодически выполняет DNS-резолвинг и
заполняет динамический set разрешёнными IP-адресами.
...
Локальная сторона — сервис, удалённая — FQDN. Аналогично AG → FQDN, но локальный set
...
Удалённая сторона — литеральный CIDR. В отличие от AG, не требует set —
...
Локальная сторона — сервис, удалённая — CIDR.
