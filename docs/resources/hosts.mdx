...
**Host** — ресурс, представляющий физический или виртуальный узел. IP-адреса и системная
метаинформация хоста являются read-only полями, обновляемыми через отдельные API-операции.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Обновление IP-адресов хоста (выполняется агентом)</td>
    </tr>
    <tr>
      <td><code>upd-metainfo</code></td>
      <td>Обновление системной метаинформации</td>
    </tr>
  </tbody>
</table>
...
Сам ресурс Host не создаёт nftables-объектов. IP-адреса хоста добавляются как элементы
