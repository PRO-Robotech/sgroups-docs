...
API-сервер читает один YAML-файл, путь к которому передаётся флагом `--config`.
В Kubernetes-манифестах конфиг живёт в `ConfigMap`, монтируется в pod как файл.
...
монтируется в pod в `/etc/sgroups/tls/`.

## Параметры: grpc (подключение к sg-server)

API-сервер проксирует операции в `sg-server` (или `sgroups-mock-backend` в
dev-режиме) через gRPC.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Отключить TLS до бэкенда (для in-cluster mock'а — ставить <code>true</code>)</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>—</td>
      <td>Override серверного имени для проверки SAN</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>Потолок exp-бэкоффа при reconnect</td>
    </tr>
  </tbody>
</table>
...
CA и клиентский сертификат добавляются как отдельные `Secret` и монтируются в
deployment'е API-сервера.

## Подключение к mock-бэкенду (dev)
...
теряются при перезапуске пода, продакшен-нагрузку он не выдержит.
:::
...
В Deployment'е этот ConfigMap монтируется в `/etc/sgroups/config/`, а флаг
...
`kube-apiserver` проверяет токены/сертификаты вызывающих и передаёт уже
аутентифицированный запрос в Aggregated API. Для этого SA сервера должен иметь
...
:::tip
В production вместо wildcard `"*"` лучше делать гранулярные роли по конкретным
ресурсам — например, отдельная роль на `rules` без доступа к `tenants`.
:::
...
graph LR
    ...
    CERT -->|"создаёт"| SECRET["Secret<br/>tls.crt + tls.key"]
    SECRET -->|"монтируется в"| POD["sgroups-k8s-apiserver Pod"]
    POD -->|"TLS :8443"| KUBE["kube-apiserver"]
```

Полные манифесты `cert-manager` лежат в `config/certificates.yaml`.
