...
Полный обзор наблюдаемости SGroups: метрики Prometheus для сервера и агента,
...
Трейсингом покрыты все gRPC-вызовы, SQL-запросы и межсервисное взаимодействие.
...
:::tip
В production рекомендуется формат логов `json` — он удобнее для разбора
