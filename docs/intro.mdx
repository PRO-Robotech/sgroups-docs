...
В современных инфраструктурах количество сервисов и хостов непрерывно растёт.
...
| **PostgreSQL** | Надёжное хранилище конфигурации с поддержкой миграций |
...
| **AddressGroup** | Центральная сущность — объединяет хосты, сети и сервисы через привязки; задаёт `default_action` (ALLOW/DENY), логирование и трейсинг |
...
| **Rule** | Правило сетевой политики — action (ALLOW/DENY), direction (ingress/egress), локальный и удалённый endpoints, транспорт |
...
| OpenTelemetry | Распределённый трейсинг (OTLP-экспорт) |
